...
import (
	...
	"kylin-ccm/pkg/cluster/kubernetes/tmpl"
	"kylin-ccm/pkg/plugins/dns"
	"kylin-ccm/pkg/util"
...
func GetClusterStatus(mgr *manager.Manager) error {
	logs.MyLogger.Infoln("Get cluster status")
...
func InitKubernetesCluster(mgr *manager.Manager) error {
	logs.MyLogger.Infoln("Initializing kubernetes cluster")
...
func JoinNodesToCluster(mgr *manager.Manager) error {
	logs.MyLogger.Infoln("Joining nodes to cluster")
