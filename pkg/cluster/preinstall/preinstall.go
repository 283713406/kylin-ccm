...
func FilesDownloadHttp(mgr *manager.Manager, filepath, version, arch string) error {
	...
	for _, binary := range binaries {
		logs.MyLogger.Infoln(fmt.Sprintf("Downloading %s, url is %s ...", binary.Name, binary.Url))
	...
	if mgr.Cluster.KubeSphere.Version == "v2.1.1" {
		logs.MyLogger.Infoln(fmt.Sprintf("Downloading %s ...", "helm2"))
...
func Prepare(mgr *manager.Manager) error {
	logs.MyLogger.Infoln("Downloading Installation Files")
	cfg := mgr.Cluster
